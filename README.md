...
食衡参加 2026 首届 openvela AI 硬件开发者大赛“快应用 / 手表应用创新”方向。它围绕“到饭点提醒 → 记录一餐 → 计算热量 → 结合补录运动与健康状态给出建议”形成离线可用的完整闭环。

## 核心功能

- 首次设置体重、每日饮食参考目标和三餐时间段；目标是食物摄入参考值，不是运动消耗任务。
...
- 首页“食衡三环”展示饮食目标进度、正餐记录进度和已记录食品多样性；运动补录只影响估算净摄入。
- 最近 7 天展示有记录天数、记录日平均摄入、食品种类以及每日摄入、补录和净摄入。
- 六类运动手动补录，按 MET 公式估算消耗，并明确标注数据来源和“估算”。
...
所有热量、运动和健康信息仅供生活管理参考，不构成医疗、诊断、治疗或减重建议。

## 项目边界

当前比赛版本不实现腕上录音、云端 ASR、手机伴侣、蓝牙同步、云账号、拍照识餐、系统级灵动岛或后台常驻。比赛开放的 `service.health` 当前只提供心率、血氧和压力，无法读取系统步数、运动记录或活动热量，因此运动消耗使用用户手动补录的 MET 估算，不用假数据冒充自动同步。
...
THIRD_PARTY_NOTICES.md           食品数据与运动常量来源说明
...
`npm run build` 会生成 debug RPK，并自动审计七个路由页面是否均进入包内。生产构建需要将自己的测试签名材料放在被忽略的 `quickapp/shihe/sign/` 目录，再执行：
...
首次启动进入设置页。保存体重、目标和餐时后进入首页；比赛健康镜像会持续提供官方心率、血氧和压力 Mock 数据。
...
生产 RPK 已用上述路径实际启动。当前开源 goldfish 运行时未注册 `system.router`，因此完整七页业务流程以 AIoT 比赛模拟器为准；goldfish 用于验证生产 RPK 可启动、VelaClaw 调用和本地降级。详细结果见 [测试报告](docs/test-report.md) 和 [能力矩阵](docs/capability-matrix.md)。
...
审计覆盖源码与包内 manifest 一致性、七页 bundle、RPK ZIP 完整性、生产签名标记、许可证、AI 日志、自建 Skill，以及高置信度密钥和私钥模式。
...
项目源码以 [Apache License 2.0](LICENSE) 发布。食品参考值、MET 产品常量及使用边界见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。
