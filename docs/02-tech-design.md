...
六个路由页面保持简单：入口、首次设置/设置、首页、记餐、确认和历史。一句话记餐作为记餐页默认模式，最近项、收藏和目录位于备用的快捷选择模式；建议、餐时胶囊和系统健康数据卡内嵌首页；日明细在历史页内展开。
...
│   ├── energy-domain.js     # 摄入进度、连续记录与 7 天摘要
...
默认 `dailyTargetKcal = 2000`，仅作成人食品营养标签对照值；编辑器产品范围为 1200–4000 kcal、步长 100 kcal，低于 1500 kcal 显示专业指导提示。V1 持久化校验继续兼容历史 800–5000 kcal；历史值只在进入编辑器时定位到最近边界，用户保存后才更新。默认餐段为早餐 07:00–09:30、午餐 11:30–14:00、晚餐 17:30–20:30。新建 Profile 不采集体重、年龄、年龄段或身高；旧对象如含 `weightKg` 仅为兼容保留，新计算不使用。
...
```text
exercises: LegacyExerciseRecordV1[]
```

为避免已安装 RC 中的 `shihe_state_v1` 被判为损坏，状态校验仍接受旧运动数组和旧记录形状。新版无运动入口，不新建、展示或聚合这些记录，也不迁移存储版本。
...
ShiheStateV1 {
  ...
  meals: MealRecordV1[], exercises: LegacyExerciseRecordV1[], // 仅兼容旧值
...
intakeProgressPercent = round(intake / dailyTargetKcal * 100)
intakeProgressCappedPercent = min(100, max(0, intakeProgressPercent))
```

首页以“今日已记录”为主数字，紧邻显示每日饮食参考线。仅使用一条摄入进度条；超过 100% 时视觉宽度封顶，同时保留真实百分比并显示高出千卡。三餐只显示“已记/待记录”文字状态。连续记录按“本地日至少有一餐”计数：今日有记录则从今日向前，否则从昨日向前，遇到首个空日停止。7 天视图含今天在内连续 7 个本地自然日，只计算有记录天数、有记录日平均摄入与每日餐数；空日补零但不进入平均值分母。旧运动数组对所有结果无影响。
...
长期产品方向是在官方提供可靠读取接口后接入系统步数、运动记录或活动热量。本次初赛不生成运动 Mock、不从健康或原始传感器数据反推热量，也不提供容易被误解为系统记录的手动运动补录。
...
- 未完成首次设置时只进入 onboarding；饮食参考线或餐段无效、免责声明未接受时不得完成。
- 编辑只改 draft，返回即丢弃；确认页从不可变 draft 渲染并重算。
- 保存生成唯一 ID、幂等写入并读取回验，成功后才导航；双击只写一条。
- 餐食记录提供修改与删除。修改复用确认流程；删除需确认。成功后 store 立即回算首页与历史。
...
- 视觉采用接近纯黑表盘背景、深灰卡片、米白主文字、灰色辅助文字、暖橙主操作与饮食状态、绿色完成状态、大数字和统一圆角。首屏只保留“记录一餐”作为强主操作。以 466×466 比赛圆屏为主验收，同时检查 480×480、方屏和 336 宽窄屏布局不溢出。

## 9. 测试计划

- 目录：恰好 60 项；稳定唯一 ID；字段完整；每项有来源；仅 `energyKcalPer100` 为能量事实源；第三方清单完整。
- 食品公式：g/ml 两类、默认份量、边界值、四舍五入及合计误差不超过 1 kcal。
- 设置：新用户无需体重；编辑器 1200–4000、步长 100、低于 1500 提示；历史 800–5000 值和可选旧体重字段读取兼容且不静默迁移。
...
- 首页聚合：今日已记录摄入、饮食参考线差值、进度封顶、三餐状态和连续记录正确；摄入 1200、参考线 2000 时差值为 800、进度为 60%，旧运动数据不改变任一结果。
- 7 天摘要：有记录天数、仅有记录日参与的平均摄入、每日餐数正确；空日不进入平均值分母。
...
- 环境：现有普通 `vela-watch-5` VVD 验 P0，不据此宣称 health；比赛镜像 VVD 验 `service.health`；当前开源 goldfish 虽配置 `ROUTER=y` 但运行时未注册 `system.router`，故以 index 降级页验 RPK 与 VelaClaw 无 Key 回退，完整六页流程以 AIoT 比赛模拟器为准；真机非初赛门禁，Mock 不算真机。
