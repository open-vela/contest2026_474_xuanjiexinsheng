...
- 压力为 42 时，本地建议自动切换为“先慢慢呼吸一分钟”方向。离开首页进入手动补录运动页再返回后，三项健康数据重新刷新。
...
1. 在 `shihe-health-watch` 启动食衡 debug RPK，确认自动订阅成功、三项官方 Mock 数据出现；进入手动补录运动页再返回首页，确认数据重新刷新，并检查压力值触发的本地建议方向。
...
| --- | --- | --- | --- | --- |
| 本地存储（`system.storage`） | P0，已实现 | 比赛 VVD 已验证首次设置、餐食和运动持久化，冷重启后保留 | 写失败保留草稿并明确提示；内存态只用于继续演示 | 是；已通过 RC1 验证 |
| 餐时胶囊 | P0，已实现 | 应用打开/恢复计算 pending/later/skipped/completed，已记录时显示餐次与热量 | 无系统闹钟时仍按本地状态在下次打开/恢复判断 | 是；已通过功能测试 |
| 最近吃过 / 收藏套餐 | P0，已实现 | 最近项、整餐收藏和分类选择已进入七页业务流程 | 目录选择；仍须确认后保存 | 是；已通过构建与模拟器验证 |
...
| 今日三环与 7 天摘要 | P0，已实现 | 饮食环、三餐记录环和食品多样环均只使用已记录业务数据；7 天摘要计算有记录天数、有记录日平均摄入和去重食品数。466×466 保存 676 kcal 套餐后实测为 34%、1/3、4/12，周摘要为 1/7、676、4/25 | 不生成综合健康分；数据不足时如实显示零值与记录完整度提示 | 是；纯计算测试与 466×466 运行通过 |
| 餐食/运动修改与删除 | P0，已实现 | 首页和历史即时回算、冷重启保留已通过模拟器验收 | 操作失败保留原记录并提示 | 是；已通过模拟器验证 |
| 运动数据来源 | P0，手动补录降级 | 本次比赛 `service.health` 仅开放 HEART_RATE/SPO2/STRESS，无法取得系统步数、运动记录或活动热量汇总 | 标题和入口明示“手动补录运动”，页面说明无法读取系统步数或运动记录；不生成 Mock、不宣称同步 | 是 |
| goldfish RPK | P0 部署门禁 | 历史 production RC1 已解包推送并由 `vapp` 启动；日志确认包名/版本、storage、VelaClaw 与 index ready。该 RPK 早于最终定版源码，不作为最终交付包 | index 明示环境边界并提供 VelaClaw 无 Key 降级验证；最终 release 须在界面审核后重新生成并复测 | 是；RC1 启动通过，最终 release 待生成 |
...
- 历史 RC1 production RPK 已在 AIoT 比赛健康镜像完成当时版本的业务验收，并在 goldfish 完成解包、推送和 index 启动；最终定版源码的 production RPK 必须在用户界面审核后重新生成并复测。初赛不要求真机，任何模拟器结论也不外推为真机验证。
...
- 视觉以比赛镜像的 466×466 圆屏为优先验收环境：接近纯黑背景、深灰卡片、暖橙主操作、绿色完成状态、低饱和蓝多样性指标、大数字与统一圆角；该尺寸关键业务流程已完成运行截图。480×480 方屏和 336×480 窄屏首次设置烟测无横向溢出，七页全量三尺寸复核仍是发布冻结门禁。
